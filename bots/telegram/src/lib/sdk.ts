/**
 * SeiMoney SDK integration for Telegram Bot
 */
...
export class SeiMoneyBotSDK {
  ...
  async initialize(mnemonic: string): Promise<void> {
    try {
      ...
      console.log(`✅ SDK initialized for address: ${account.address}`);
    } catch (error) {
  ...
  /**
   * Deposit to pot using Pots contract
   */
  async depositToPot(
    potId: number,
    amount: string,
    ...
    try {
      ...
      return {
        success: true,
        txHash: result.transactionHash,
        message: `💰 Deposited ${amount} ${denom} to pot #${potId}!\nTxHash: ${result.transactionHash}`
      };
    } catch (error) {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error',
        message: '❌ Failed to deposit to pot'
      };
    }
  }

  /**
   * Open escrow case using Risk Escrow contract
   */
  async openEscrowCase(
    parties: string[],
    amount: string,
    denom: string = 'usei',
    model: 'MultiSig' | 'TimeTiered' | 'Milestones' = 'MultiSig',
    expiry?: number,
    remark?: string
  ): Promise<TransactionResult> {
    if (!this.signingClient || !this.wallet) {
    ...
    try {
      ...
      const msg = {
        open_case: {
          ...
          amount: { amount, denom },
          model,
          expiry_ts: expiry || Math.floor(Date.now() / 1000) + 604800, // 7 days default
          remark
        }
      };
      ...
      const result = await this.signingClient.execute(
        account.address,
        this.config.contracts.riskEscrow,
        msg,
        'auto'
      );
      ...
      return {
        success: true,
        txHash: result.transactionHash,
        message: `🛡️ Escrow case opened successfully!\nTxHash: ${result.transactionHash}`
      };
    } catch (error) {
      console.error('❌ Failed to open escrow case:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error',
        message: '❌ Failed to open escrow case'
      };
    }
  }
