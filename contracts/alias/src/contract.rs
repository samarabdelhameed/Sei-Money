...
use cosmwasm_std::{
    ensure, to_json_binary, Binary, Deps, DepsMut, Env, MessageInfo, Response,
    StdResult, Order,
};
...
fn exec_update(
    ...
    let old_username = ADDR_TO_NAME.load(deps.storage, &info.sender)
...
fn exec_unregister(
    ...
    // Check if address has a username
    let username = ADDR_TO_NAME.load(deps.storage, &info.sender)
        .map_err(|_| NoUsernameRegistered)?;
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::Resolve { username } => {
            let addr = NAME_TO_ADDR.load(deps.storage, username.as_str())
        ...
        QueryMsg::ReverseLookup { address } => {
            ...
            let username = ADDR_TO_NAME.load(deps.storage, &addr)
        ...
        QueryMsg::ListUsernames { start_after, limit } => {
