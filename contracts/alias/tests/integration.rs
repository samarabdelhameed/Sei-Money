...
fn test_register_duplicate_username() {
    ...
    // Try to register same username with different user
    let msg = ExecuteMsg::Register {
        username: "alice".to_string(),
    };
...
fn test_register_duplicate_address() {
    ...
    // Try to register different username with same user
    let msg = ExecuteMsg::Register {
...
fn test_update_username() {
    ...
    // Query the updated username
    let query_msg = QueryMsg::Resolve { username: "alice_new".to_string() };
    let result: UsernameResp = app
        .wrap()
        .query_wasm_smart(contract_addr, &query_msg)
        .unwrap();
    
    assert_eq!(result.username, "alice_new");
    assert_eq!(result.address, USER1);
}

#[test]
fn test_unregister_username() {
    let mut app = mock_app();
    let contract_addr = setup_contract(&mut app);
    ...
    // Unregister username
    let unregister_msg = ExecuteMsg::Unregister {};
    ...
    // Query should fail
    let query_msg = QueryMsg::Resolve { username: "alice".to_string() };
    let result: Result<UsernameResp, _> = app
        .wrap()
        .query_wasm_smart(contract_addr, &query_msg);
...
#[test]
fn test_get_address_by_username() {
    let mut app = mock_app();
    let contract_addr = setup_contract(&mut app);
...
fn test_username_validation() {
    ...
    // Try to register invalid username (with spaces)
    let msg = ExecuteMsg::Register {
...
fn test_list_usernames() {
    ...
    for username in usernames.iter() {
        ...
        app.execute_contract(
            Addr::unchecked(&format!("sei1user{}", username)),
            contract_addr.clone(),
            &msg,
            &[],
        ).unwrap();
    ...
    // List all usernames
    let query_msg = QueryMsg::ListUsernames { start_after: None, limit: None };
    let result: Vec<UsernameResp> = app
        .wrap()
        .query_wasm_smart(contract_addr, &query_msg)
        .unwrap();
